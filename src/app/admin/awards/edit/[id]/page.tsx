...
export default function AdminAwardsEditPage({
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [yearsRes, catRes, nomineeRes] = await Promise.all([
                    fetch("/api/awards/years").then(res => res.json()),
                    fetch("/api/awards/categories").then(res => res.json()),
                    fetch(`/api/awards/nominees/${id}`).then(res => res.json())
                ]);
                ...
                alert("Could not load nominee data. It may have been deleted.");
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch(`/api/awards/nominees/${id}`, {
                method: "PUT",
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.error?.message || "Failed to update nominee");
            }
    ...
    return (
        <div className="space-y-8 max-w-4xl">
            <div className="pb-6 border-b border-gray-200 dark:border-zinc-800">
                ...
                <h1 className="text-3xl font-bold font-thai tracking-tight text-black dark:text-white uppercase">Edit Nominee</h1>
                <p className="text-gray-500 mt-2 font-thai">แก้ไขข้อมูลผู้เข้าชิงหรือตั้งเป็นผู้ชนะ</p>
            </div>
