...
import { ArrowRight, Clock, ChevronLeft, ChevronRight, FolderOpen } from "lucide-react";
...
export default async function ArticlesPage({ searchParams }: { searchParams: Promise<{ page?: string; tag?: string; category?: string }> }) {
    ...
    // Fetch articles, count, categories, and default image in parallel
    const [articlesData, totalCount, defaultImageSetting, categoriesData] = await Promise.all([
    ...
    const categories = categoriesData.map(cat => ({
        ...
        slug: cat.slug,
        name: cat.name,
        count: (cat as any)._count?.articles || 0,
    }));

    const totalArticlesCount = categories.reduce((sum, c) => sum + c.count, 0);

    const isFirstPage = page === 1;
    const featuredArticle = (isFirstPage && articles.length > 0 && !categorySlug && !tag) ? articles[0] : null;
    const remainingArticles = featuredArticle ? articles.slice(1) : articles;
    const totalPages = Math.ceil(totalCount / pageSize);

    const buildPageUrl = (p: number) => {
        const parts = [`/articles?page=${p}`];
    ...
    return (
        <div className="flex flex-col min-h-screen bg-white dark:bg-[#0a0a0a] transition-colors duration-300">
            ...
            <section className="w-full bg-gray-50 dark:bg-[#050505] py-16 md:py-24 border-b border-gray-200 dark:border-white/10">
                <div className="container px-6 mx-auto text-center">
                    ...
                    <p className="text-lg md:text-xl text-gray-600 dark:text-gray-400 font-thai max-w-2xl mx-auto">
                        {categorySlug
                            ? `แสดงบทความในหมวดหมู่: ${categories.find(c => c.slug === categorySlug)?.name || categorySlug}`
                            : tag
                                ? `แสดงบทความที่มีข้อความกำกับ: #${tag}`
            ...
            {/* FEATURED ARTICLE (Page 1 only, no filter active) */}
            {featuredArticle && (
            ...
            {/* MAIN CONTENT: Category Sidebar + Articles Grid */}
                ...
                <div className="flex flex-col lg:flex-row gap-12">
                    ...
                    <aside className="hidden lg:block w-64 shrink-0">
                        <div className="sticky top-32">
                            <h3 className="text-xs font-bold uppercase tracking-[0.2em] text-gray-400 dark:text-gray-500 mb-6 flex items-center gap-2">
                            ...
                            <nav className="space-y-1">
                                <Link
                                    ...
                                    className={`flex items-center justify-between px-4 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 ${
                                        !categorySlug
                                            ? "bg-[#cfb659] text-black font-bold"
                                            : "text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-zinc-900 hover:text-black dark:hover:text-white"
                                    }`}
                                >
                                    <span className="font-thai">ทั้งหมด</span>
                                    <span className="text-xs opacity-70">{totalArticlesCount}</span>
                                </Link>
                                {categories.map(cat => (
                                    <Link
                                        key={cat.id}
                                    ...
                                    >
                                        <span className="font-thai">{cat.name}</span>
                        ...
                        <div className="lg:hidden flex flex-wrap gap-2 mb-2">
                            <Link
                                ...
                                className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-widest transition-all border ${
                                    !categorySlug
                                        ? "bg-[#cfb659] text-black border-[#cfb659]"
                                        : "bg-transparent text-gray-500 dark:text-gray-400 border-gray-200 dark:border-zinc-800 hover:border-[#cfb659] hover:text-[#cfb659]"
                                }`}
                            >
                                ทั้งหมด
                            </Link>
                            ...
                            <>
                                <div className="flex items-center justify-between mb-8 pb-4 border-b border-gray-200 dark:border-white/10">
                                    <h3 className="text-2xl font-bold font-thai text-black dark:text-white uppercase tracking-wider">
                                        {categorySlug
                                            ? categories.find(c => c.slug === categorySlug)?.name || "Results"
                                            : tag
                                                ? `Results for #${tag}`
                                ...
                                {totalPages > 1 && (
                                    <div className="mt-16 flex items-center justify-between border-t border-gray-200 dark:border-white/10 pt-8">
                                        ...
                                        <div className="flex flex-1 sm:flex-none justify-between gap-4">
                                            <Link href={buildPageUrl(page - 1)} className="w-1/2 sm:w-auto">
                                                <Button
                                                    variant="outline"
